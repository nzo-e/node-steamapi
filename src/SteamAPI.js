...
class SteamAPI {
	 ...
	 * @param {string} id User ID
	 * @param {boolean} [includeAppInfo=true] Include App Info
	 * @param {boolean} [includePlayedFreeGames=false] Include Played Free Games
	 * @returns {Promise<Game[]>} Owned games
	 */
	getUserOwnedGames(id, includeAppInfo = true, includePlayedFreeGames = false) {
		if (!reID.test(id)) return Promise.reject(new TypeError('Invalid/no id provided'));
		if (!(typeof(includeAppInfo)==='boolean')) return Promise.reject(new TypeError('Invalid include_appinfo provided'));
		if (!(typeof(includePlayedFreeGames)==='boolean')) return Promise.reject(new TypeError('Invalid include_played_free_games provided'));


		return this
			.get(`/IPlayerService/GetOwnedGames/v1?steamid=${id}&include_appinfo=${includeAppInfo?'1':'0'}&include_played_free_games=${includePlayedFreeGames?'1':'0'}`)
